...
class MakeMainFulljauscms extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'fulljauscms:main {module=System : Nombre del modulo donde crearemos el model} {model=SysUser : Nombre del modelo a crear} {modelplural=SysUsers : Nombre del modelo en plural para crear las tablas} {del? : Elimina el modelo}';
     ...
     * @var string
     */
    protected $description = 'Comando Fulljaus Cms para crear un nuevo modelo de datos dentro de Main. Crea el Controlador, las vistas de form y listado y el modelo + la migracion. El nombre del modelo se ingresa en Singular y se crea la tabla en plural';
    ...
    public function handle()
    {
        ...
        $pathModule = base_path("main/".$moduleName.'/');
        ...
        $filenameMigration=base_path('database/migrations/').date('Y_m_d').'_000000_create_'.$tableName.'_table.php';
        $filenameController=$pathModule.'Controllers/'.$controllerName.'.php';
        $filenameSheet=$pathModule.'Controllers/'.$modelName.'Sheet.php';
        $filenameSeeder=base_path('database/seeders/').$seederName.'Seeder.php';
        $filenameViewList=$pathModule.'Views/'.strtolower($modelName).'list.blade.php';
        $filenameViewEdit=$pathModule.'Views/'.strtolower($modelName).'edit.blade.php';
        ...
        $contentModel=str_replace('{{ namespace }}','Main\\'.$moduleName.'\\Models',$contentModel);
        ...
        $contentSheet=str_replace('{{ namespace }}','Main\\'.$moduleName.'\\Controllers',$contentSheet);
        $contentSheet=str_replace('{{ model }}','Main\\'.$moduleName.'\\Models\\'.$modelName,$contentSheet);
        ...
        // Reemplazo el contenido
        $contentController=str_replace('{{ namespace }}','Main\\'.$moduleName.'\\Controllers',$contentController);
        $contentController=str_replace('{{ model }}','Main\\'.$moduleName.'\\Models\\'.$modelName,$contentController);
        ...
        $contentController=str_replace('{{ viewmain }}',$moduleName.'.Views.'.strtolower($modelName).'list',$contentController);
        $contentController=str_replace('{{ viewedit }}',$moduleName.'.Views.'.strtolower($modelName).'edit',$contentController);
        ...
        $instructions.='- Edite el archivo routes/web.php y agregue:'.PHP_EOL;
        $instructions.='// '.$moduleName.' - '.$modelName.PHP_EOL;
        $instructions.="Route::get('/".strtolower($readPlural)."', 'Main\\".$moduleName."\Controllers\\".$modelName."Controller@index')->name('".$routeroot.".list');".PHP_EOL;
        $instructions.="Route::get('/nuevo/".strtolower($readSingular)."', 'Main\\".$moduleName."\Controllers\\".$modelName."Controller@create')->name('".$routeroot.".create');".PHP_EOL;
        $instructions.="Route::post('/ordenar/".strtolower($readPlural)."', 'Main\\".$moduleName."\Controllers\\".$modelName."Controller@sorting')->name('".$routeroot.".sort');".PHP_EOL;
        $instructions.="Route::get('/editar/".strtolower($readSingular)."/{id}', 'Main\\".$moduleName."\Controllers\\".$modelName."Controller@edit')->name('".$routeroot.".edit');".PHP_EOL;
        $instructions.="Route::post('/eliminar/".strtolower($readSingular)."', 'Main\\".$moduleName."\Controllers\\".$modelName."Controller@delete')->name('".$routeroot.".delete');".PHP_EOL;
        $instructions.="Route::post('/guardar/".strtolower($readSingular)."', 'Main\\".$moduleName."\Controllers\\".$modelName."Controller@save')->name('".$routeroot.".save');".PHP_EOL;
        $instructions.="Route::post('/obtener/".strtolower($readPlural)."', 'Main\\".$moduleName."\Controllers\\".$modelName."Controller@getdata')->name('".$routeroot.".getdata');".PHP_EOL;
        $instructions.="Route::get('/exportar/".strtolower($readPlural)."', 'Main\\".$moduleName."\Controllers\\".$modelName."Controller@sheet')->name('".$routeroot.".sheet');".PHP_EOL;
